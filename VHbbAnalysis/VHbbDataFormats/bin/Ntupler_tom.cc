...
template <> void LeptonInfo::setSpecific<VHbbEvent::ElectronInfo>(const VHbbEvent::ElectronInfo & i, int j,const VHbbEventAuxInfo & aux){
  ...
  id80[j]=i.tight;
  //  id95[j]=i.id95;
  id95[j]=i.robustTight;
  // TOM
  /* id80NoIso[j]=(i.innerHits ==0 && !(fabs(i.convDist)<0.02 && fabs(i.coxnvDcot)<0.02) &&
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "OUR CODE STARTS HERE"<<std::endl;
  ...
  for(unsigned int iFile=0; iFile<inputFiles_.size(); ++iFile) {
	...
	if (ievt > maxEvents_ + skipEvents_) break;
      ...
      if (totalCount %100 == 0)
	std::cout <<" VHBBEVENT: "<<totalCount<<std::endl;
	...
	lheV_pt = V_tlv.Pt();
      ...
      // simBHadrons
      // TOM      const SimBHadronCollection *sbhc;
      // TOM HERE I NEED TO CHANGE: PRODUCE SIMBHADRON ON THE FLY


      if(isMC_){
	// TOM	fwlite::Handle<SimBHadronCollection> SBHC;
...
TOM	*/
	...
	modifiedEvent = hbbInfo;
//	std::cout<<" MODIFIED"<<std::endl;
//	std::cout <<" A JET! "<< modifiedEvent.simpleJets2.size() << " " <<aux.mcH.size() <<std::endl;
...
//      std::cout<<"after continue"<<std::endl;
	...
	MHT.sig = iEvent->mht.metSig; 
      ...
      TkSharing.HiggsCSVtkSharing = false;
      TkSharing.HiggsIPtkSharing = false;
      TkSharing.HiggsSVtkSharing = false;
      TkSharing.FatHiggsCSVtkSharing = false;
      TkSharing.FatHiggsIPtkSharing = false;
	...
	if (vhCand.H.jets[0].csvNTracks > 0 && vhCand.H.jets[1].csvNTracks > 0){
	  for (int t=0;t<vhCand.H.jets[0].csvNTracks;t++){
	    for (int ti=0;ti<vhCand.H.jets[1].csvNTracks;ti++){
	      std::cout <<" JETS"<<vhCand.H.jets.size()<<std::endl;
	      std::cout <<" ECCO "<<vhCand.H.jets[0].csvTrackIds.size()<<" " <<vhCand.H.jets[1].csvTrackIds.size()<<" "<< t<<" " <<ti<<std::endl;
	...
	*/
	...
	for(unsigned int j=0; j < vhCand.additionalJets.size(); j++ ){
	  if (vhCand.additionalJets[j].csv > maxBtag) { 
	    bJet=vhCand.additionalJets[j].p4 ; maxBtag =vhCand.additionalJets[j].csv; 
	  }
	...
	for(unsigned int j=0; j < iEvent->simpleJets2.size(); j++ ){
	  if (iEvent->simpleJets2[j].csv > maxBtag) { 
	    bJet=iEvent->simpleJets2[j].p4 ; maxBtag =iEvent->simpleJets2[j].csv;
	  }
